...
class MethaneLoader(DataLoader):
    ...
    def __init__(self, device, mode, plume_id, red=False, alli=False, channels=12):
        ...
        if plume_id is not None:
            self.pos_labels = sorted(
                glob("final_annotations/plume_{}/{}/pos/*.npy".format(plume_id, mode))
            )
            self.neg_labels = sorted(
                glob("final_annotations/plume_{}/{}/neg/*.npy".format(plume_id, mode))
            )
        else:
            self.pos_labels = sorted(
                glob("final_annotations/plume_*/{}/pos/*.npy".format(mode))
            )
            self.neg_labels = sorted(
                glob("final_annotations/plume_*/{}/neg/*.npy".format(mode))
            )
    ...
    def __getitem__(self, index):
        ...
        target = np.load(f)
        context = np.load(
            "turkmenistan_plumes_raw/plume_{}/{}-raw.npy".format(plume_id, date)
        )
        ...
        # diff_img = np.array(
        # diff_img[mid_loc_x - s : mid_loc_x + s, mid_loc_y - s : mid_loc_y + s, :]
        # )
        # diff_img_g = np.array(
        #     diff_img_g[mid_loc_x - s : mid_loc_x + s, mid_loc_y - s : mid_loc_y + s]
        # )
        # rgb_img = np.array(
        #     rgb_img[mid_loc_x - s : mid_loc_x + s, mid_loc_y - s : mid_loc_y + s, :]
        # )
        ...
        d = {
            "pred": torch.from_numpy(context).float().to(self.device).permute(2, 0, 1)
            / 255,
...
class MethaneLoaderNew(DataLoader):
    ...
    def __init__(self, device, mode, plume_id, red=False, alli=False, channels=12):
        ...
        self.reduce = red
        self.channels = channels
        self.data_info = pd.read_csv("../data/{}.csv".format(mode))
        self.pos_labels = list(self.data_info[self.data_info["flag"] == 1]["index"])
        self.neg_labels = list(self.data_info[self.data_info["flag"] == 0]["index"])
        if mode == "train":
            persist = False
        else:
            persist = True
    ...
    def __getitem__(self, index):
        ...
        target = np.load("../data/{}/label/{}.npy".format(self.mode, index_new))
        context = np.load("../data/{}/s2/{}.npy".format(self.mode, index_new))
...
class MethaneLoaderNewQ(DataLoader):
    ...
    def sample_labels_and_combine(self, persist=False):
        ...
        """
        # if self.mode == "test":
        if self.mode in ["test", "val", "train"]:
            self.labels = self.pos_labels + self.neg_labels
        else:
            if persist:
                random.seed(555)
    ...
    def __getitem__(self, index):
        index_new = int(self.labels[index])
        target = np.load("../data/{}/mbmp/{}.npy".format(self.mode, index_new))
        context = np.load("../data/{}/s2/{}.npy".format(self.mode, index_new))
